...
export const SecurityControlsGrid = ({ domains, theme = 'dark', detailed = false }: SecurityControlsGridProps) => {
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-2">
        ...
        <p className={`text-sm ${textSecondary} max-w-3xl leading-relaxed font-light`}>
          {detailed 
            ...
            : "High-level overview of security domains and compliance validation status."}
        </p>
      </div>

      {/* Horizontal Domain Tabs - ONLY render tab group if in detailed view */}
      {detailed ? (
        ...
        <div className="text-xs font-bold uppercase tracking-wider text-brand-orange bg-brand-orange/5 px-4 py-2 rounded-xl border border-brand-orange/10 w-fit">
          Standard Security Profiles Active
        </div>
      )}
      ...
      {selectedDomain && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 animate-in fade-in slide-in-from-bottom-3 duration-400">
          {/* If in Normal view, we render controls from all domains or just the primary domain, but simplified */}
          {(detailed ? selectedDomain.controls : domains.flatMap(d => d.controls).slice(0, 4)).map((control) => {
            ...
            const cardStyles = isOk 
              ? isDark 
                ? 'bg-zinc-950/40 border-green-500/20 hover:border-green-500/40 shadow-[0_2px_12px_rgba(34,197,94,0.01)]' 
                : 'bg-zinc-50/60 border-green-500/20 hover:border-green-500/40 shadow-[0_2px_12px_rgba(34,197,94,0.01)]'
              : isNotOk 
              ? isDark
                ? 'bg-zinc-950/40 border-brand-red/35 hover:border-brand-red/50 shadow-[0_2px_12px_rgba(255,57,24,0.02)]' 
                : 'bg-zinc-50/60 border-brand-red/25 hover:border-brand-red/45 shadow-[0_2px_12px_rgba(255,57,24,0.02)]'
              : isDark
                ? 'bg-zinc-950/40 border-brand-orange/30 hover:border-brand-orange/50 shadow-[0_2px_12px_rgba(255,138,28,0.01)]'
                : 'bg-zinc-50/60 border-brand-orange/25 hover:border-brand-orange/45 shadow-[0_2px_12px_rgba(255,138,28,0.01)]';
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="space-y-1.5">
                    ...
                    {/* Render technical remarks ONLY in detailed view */}
                    {detailed && control.remarks && (
                ...
                {/* Footer Metadata row - ONLY in detailed view */}
                {detailed && (
