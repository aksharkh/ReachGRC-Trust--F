...
export const Preloader: React.FC<PreloaderProps> = ({ isLoading }) => {
  ...
  return (
    <div 
      className={`fixed inset-0 z-50 flex flex-col items-center justify-center bg-zinc-50 dark:bg-black transition-opacity duration-500 ${fade ? 'opacity-0' : 'opacity-100'}`}
    >
      <div className="relative flex flex-col items-center">
        ...
        <div className="relative">
          <Shield className="w-20 h-20 text-brand-orange animate-[pulse_2s_cubic-bezier(0.4,0,0.6,1)_infinite]" strokeWidth={1.5} />
          {/* Scanning line animation */}
          <div className="absolute inset-0 overflow-hidden rounded-full">
            <div className="w-full h-1 bg-brand-orange/50 dark:bg-white/50 blur-[2px] animate-[scan_2s_ease-in-out_infinite]"></div>
          </div>
        </div>
        
        <div className="mt-8 space-y-3 text-center z-10">
          <h2 className="text-2xl font-light tracking-[0.2em] text-zinc-900 dark:text-white uppercase">Reach<span className="font-bold">GRC</span></h2>
          ...
          <p className="text-xs text-zinc-400 dark:text-zinc-500 tracking-widest uppercase mt-4">Establishing Secure Connection</p>
        </div>
      </div>
