...
const ApiKeyPreview = () => {
  ...
  const [keyData, setKeyData] = useState({
    key: "rgc_a3f9c2e1b74d6085fa91dc23e0b58764",
    status: "Active",
    issued: "Issued: Jan 15, 2025",
    expires: "Expires: Jan 15, 2026"
  });

  useEffect(() => {
    fetch("http://localhost:8081/api/trust/1")
      .then(res => res.json())
      .then(data => {
        if (data && data.apiKey) {
          setKeyData({
            key: data.apiKey,
            status: data.apiKeyStatus || "ACTIVE",
            issued: data.apiKeyIssuedAt ? `Issued: ${new Date(data.apiKeyIssuedAt).toLocaleDateString()}` : "Issued: Jan 15, 2025",
            expires: data.apiKeyExpiresAt ? `Expires: ${new Date(data.apiKeyExpiresAt).toLocaleDateString()}` : "Expires: Jan 15, 2026"
          });
        }
      })
      .catch(() => {});
  ...
  const masked = keyData.key.slice(0, 4) + "•".repeat(Math.max(0, keyData.key.length - 4));
  ...
  const getStatusBadge = () => {
    const status = keyData.status.toUpperCase();
    if (status === "ACTIVE") {
      return (
        <Badge variant="green">
    ...
    <div className="border border-zinc-200 dark:border-zinc-700 rounded-lg overflow-hidden">
      ...
      <div className="px-4 py-3 flex items-center justify-between gap-3">
        <span className="font-mono text-sm text-zinc-700 dark:text-zinc-300 tracking-wide truncate">
          {visible ? keyData.key : masked}
        </span>
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <button
            onClick={() => setVisible((v) => !v)}
            className="p-1.5 rounded-md text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors cursor-pointer"
      ...
      <div className="px-4 py-2.5 bg-zinc-50 dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-700 flex items-center justify-between text-[11px] text-zinc-400">
        <span>{keyData.issued}</span>
        <span>{keyData.expires}</span>
      </div>
    </div>
  );
};
