...
import { PageFeedback } from "@/components/docs/ReusableComponents";

// Data contracts for documentation video tutorials
interface Tutorial {
  id:          string;
  title:       string;
  description: string;
  ...
  >
    ...
    >
      ...
      <div className="px-5 py-4 flex items-start justify-between gap-4">
        ...
        <button
          onClick={onClose}
          className="flex-shrink-0 text-xs text-zinc-400 hover:text-zinc-650 dark:hover:text-zinc-200 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-1.5 cursor-pointer transition-colors"
        >
          Close
        </button>
      </div>
...
// Ordered step-by-step checklist below video summaries
...
// Individual alternating tutorial cards (thumbnail left/right alternating layout)
const TutorialRow = ({ tutorial, index, onClick }: { tutorial: Tutorial; index: number; onClick: () => void }) => {
  ...
  const Info = (
    <div className="flex flex-col justify-center flex-1 min-w-0 py-2">
      <span className="text-[11px] font-bold tracking-widest text-zinc-300 dark:text-zinc-650 mb-2 uppercase select-none">
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center gap-6 py-6 border-b border-zinc-100 dark:border-zinc-850 last:border-0">
      {isEven ? (
        <>
          {Info}
          {Thumbnail}
...
// Collapsible grouping drawer containing tutorial details
...
/**
 * TutorialsPage renders the interactive video training guides.
 * 
 * Features:
 *   - Alternating block rows with YouTube thumbnail pre-fetch and click-to-play overlay
 *   - Video modals overlays with autoplay and title descriptions
 *   - Difficulty badges (beginner, intermediate, advanced)
 *   - Step-by-step audit checklists for each tutorial
 *   - Collapsible groups matching key modules (Quickstart, Controls, API/Jira integration, branding)
 *   - Standalone Page Feedback response collection widget at the bottom
 */
const TutorialsPage = () => (
  <div className="h-full overflow-y-auto bg-white dark:bg-zinc-950">
    <div className="max-w-4xl mx-auto px-6 py-10">
      ...
      {/* Header Descriptions */}
      <div className="mb-10">
        <p className="text-[11px] font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-550 mb-2 select-none">
          Tutorials
        </p>
      ...
      <div className="flex items-center gap-4 mb-8 flex-wrap">
        <span className="text-xs text-zinc-400 dark:text-zinc-500 font-medium select-none">Difficulty:</span>
        {["beginner", "intermediate", "advanced"].map((t) => <Tag key={t} tag={t} />)}
      </div>

      <hr className="border-zinc-150 dark:border-zinc-850 mb-10" />

      {/* Accordion List Sections */}
      {groups.map((g) => <GroupSection key={g.id} group={g} />)}

      {/* Integrated interactive feedback section */}
      <PageFeedback />
